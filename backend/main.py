...
def log_health(health: schemas.HealthCreate, db: Session = Depends(get_db)):
...
def get_health_logs(skip: int = 0, limit: int = 7, db: Session = Depends(get_db)):
...
def add_debt(debt: schemas.LearningDebtCreate, db: Session = Depends(get_db)):
...
def log_career_output(output: schemas.CareerCreate, db: Session = Depends(get_db)):
...
def get_career_outputs(db: Session = Depends(get_db)):
