...
def delete_daily_journal(db_session, target_date: date):
    try:
        journal = db_session.query(DailyJournal).filter(DailyJournal.date == target_date).first()
        if journal:
...
def delete_weekly_review(db_session, target_date: date):
    try:
        week_start = target_date - timedelta(days=target_date.weekday())
        review = db_session.query(WeeklyReview).filter(WeeklyReview.week_start == week_start).first()
        if review:
...
def delete_monthly_reflection(db_session, year: int, month: int):
    try:
        ym = f"{year}-{month:02d}"
        reflection = db_session.query(MonthlyReflection).filter(MonthlyReflection.year_month == ym).first()
        if reflection:
            db_session.delete(reflection)
            db_session.commit()
    except Exception as e:
        db_session.rollback()
        raise e
